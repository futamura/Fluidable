...
- `ios create_doc` は Swift-DocC で `docs/` に static hosting docs を生成する。
- `docs/` の Jazzy 生成物を DocC 生成物に置換し、公開 docs の古い Carthage / CocoaPods 表記を撤去した。
- agent memory / superpowers plan は `.agents/` 配下に移動し、`docs/` は DocC 生成物専用にした。
- `Gemfile.lock` から Jazzy / Slather / CocoaPods 系の直接依存を撤去した。
...
- Create: `.agents/memory/project_xcode26_spm_migration.md`
...
Update `.agents/memory/MEMORY.md` Projects section:
...
- .agents/memory/project_xcode26_spm_migration.md
